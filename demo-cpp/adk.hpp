...
PosixSocket::PosixSocket( int internal_socket ) : internal_socket( internal_socket )
{}
...
bool PosixSocket::send( char *msg, size_t len )
{
	return ::send( internal_socket, msg, len, 0 ) != -1;
}

bool PosixSocket::recv( char *buf, size_t len )
{
	return ::recv( internal_socket, buf, len, 0 ) == len;
}
...
Channel *socket_channel( const std::string &host, unsigned int port )
{
	...
	if ( connect( sock, (struct sockaddr *) &addr, sizeof( addr ) ) == -1 ) return nullptr;
...
WindowsSocket::WindowsSocket( SOCKET internal_socket ) : internal_socket( internal_socket ) {}
...
bool WindowsSocket::send( char *msg, size_t len )
{
	return ::send( internal_socket, msg, len, 0 ) != SOCKET_ERROR;
}

bool WindowsSocket::recv( char *buf, size_t len )
{
	return ::recv( internal_socket, buf, len, 0 ) == len;
}
...
bool StdIO::send( char *msg, size_t len )
{
...
bool StdIO::recv( char *buf, size_t len )
{
	size_t cnt = 0;
	while ( cnt < len ) {
		int c = getchar();
...
public:
	...
	TwoDimArray( const TwoDimArray &other ) : length( other.length ), width( other.width ), data( new T[length * width] )
	{
		std::copy( std::begin( other.data ), std::end( other.data ), std::begin( data ) );
	}
	TwoDimArray( TwoDimArray &&other ) = delete;

	RowAccessor<T> operator[]( size_t row )
	{
		return RowAccessor<T>( row * length, data );
	}
	const RowAccessor<T> operator[]( size_t row ) const
	{
		return RowAccessor<T>( row * length, data );
	}
...
struct Snake {
	...
	{
		return coord_list[idx];
	}
};

bool operator==( const Snake &lhs, const Snake &rhs )
{
...
Context::Context( int length, int width, int max_round, std::vector<Item> &&item_list )
	: _length( length ), _width( width ), _max_round( max_round ), _current_round( 0 ), _current_player( 1 ),
	  _wall_map{ static_cast<size_t>( length ), static_cast<size_t>( width ), -1 },
...
const std::vector<Snake> &Context::my_snakes() const
{
	return _current_player == 0 ? _snake_list_0 : _snake_list_1;
}

std::vector<Snake> &Context::my_snakes()
{
	return _current_player == 0 ? _snake_list_0 : _snake_list_1;
}

const std::vector<Snake> &Context::opponents_snakes() const
{
	return _current_player == 0 ? _snake_list_1 : _snake_list_0;
}

std::vector<Snake> &Context::opponents_snakes()
{
	return _current_player == 0 ? _snake_list_1 : _snake_list_0;
}

const Item &Context::find_item( int item_id ) const
{
	return _item_list[item_id];
}

const Snake &Context::find_snake( int snake_id ) const
{
...
void Context::remove_snake( int snake_id )
{
...
void Context::flood_fill( TwoDimArray<int> &map, int x, int y, int v )
{
	std::queue<Coord> q;
	q.push( { x, y } );
	while ( !q.empty() ) {
		...
		q.pop();
		int cx = c.x, cy = c.y;
		if ( cx < 0 || cx >= _length || cy < 0 || cy >= _width || map[cx][cy] != 0 ) continue;
		map[cx][cy] = v;
		q.push( { cx + 1, cy } );
		q.push( { cx - 1, cy } );
		q.push( { cx, cy + 1 } );
...
void Context::check_dir( TwoDimArray<int> &map, bool *dir_ok, int x, int y )
{
	if ( map[x][y] == 1 ) dir_ok[1] = false;
	else if ( map[x][y] == 2 )
		dir_ok[2] = false;
}

void Context::seal_region()
{
	...
	for ( auto c: snake.coord_list ) {
		if ( x0 == c.x && y0 == c.y && !is_head ) break;
		grid[c.x][c.y] = 3;
		is_head = false;
	}
...
SnakeGoAI::SnakeGoAI( int argc, char **argv )
{
	if ( argc == 1 ) {
		ch = stdio_channel();
	} else if ( argc == 3 ) {
...
int SnakeGoAI::read_short()
{
	static char c = 0;
	if ( !ch->recv( &c, 1 ) ) crash();
	return c;
}

#define BIG_ENDIAN_INT( C1, C2 ) ( ( ( static_cast<unsigned char>( C1 ) << 8 ) | ( static_cast<unsigned char>( C2 ) ) ) & 0xFFFF )

int SnakeGoAI::read_int()
{
	static char c[] = { 0, 0 };
	if ( !ch->recv( c, 2 ) ) crash();
	return BIG_ENDIAN_INT( c[0], c[1] );
}

std::vector<Item> SnakeGoAI::read_item_list()
{
	if ( read_short() != 0x10 ) crash();

	int item_count = read_int();
	if ( item_count <= 0 ) crash();
	char *buf = new char[7 * item_count];
	if ( !ch->recv( buf, 7 * item_count ) ) crash();
...
void SnakeGoAI::handle_gameover()
{
